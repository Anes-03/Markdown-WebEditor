...
${trimmed}
  ...
  async function requestTemplateAiSnippet(topic, markdown, signal) {
    ...
    const trimmed = (markdown || '').trim();
    const hasContent = !!trimmed;
    const systemInstruction = 'Du bist eine erfahrene technische Redakteurin. Erstelle hochwertige Markdown-Vorlagen mit klaren Überschriften, Tabellen oder Listen, die direkt eingesetzt werden können. Verwende Platzhalter wie {{title}}, {{date}}, {{datetime}}, {{time}} oder {{year}}, wenn sie sinnvoll sind. Du kannst bei Bedarf auch Mermaid-Diagramme (```mermaid```) einbetten, z. B. flowchart, sequenceDiagram, mindmap oder journey, sofern sie den Inhalt unterstützen. Gib ausschließlich die fertige Markdown-Vorlage ohne zusätzliche Erklärungen aus.';
